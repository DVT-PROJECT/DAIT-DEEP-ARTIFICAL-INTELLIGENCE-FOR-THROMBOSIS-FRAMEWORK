...
type AnalyticsOut = { metrics: Metrics; timeline: TimelinePoint[] };
...
export function Dashboard({ auth, onLogout }: { auth: AuthState; onLogout: () => void }) {
  ...
  async function refreshMetrics() {
    try {
      ...
      const out = data as AnalyticsOut;
  ...
  async function runPrediction() {
    ...
    try {
      ...
      const { data } = await api.post('/studies/predict', form, {
  ...
  async function tuneModelAutomatically() {
    try {
      await api.post('/tuning/run', { epochs: 2, batch_size: 8 });
    } catch {
      return;
    }
  }
  ...
  async function removePatient(patientIdToRemove: number) {
    ...
    setBusy(true);
    try {
      await api.delete(`/patients/${patientIdToRemove}`);
      await refreshHistory();
      await refreshMetrics();
    } catch (e: any) {
      setErr(e?.response?.data?.detail ?? 'Failed to remove patient');
    } finally {
      setBusy(false);
    }
  }
                    ...
                    <TableCell align="right">
                      <Stack direction="row" spacing={1} justifyContent="flex-end">
                        <IconButton
                          ...
                          onClick={() => r.last_study_id && window.open(`${API_BASE_URL}/studies/${r.last_study_id}/report.pdf`, '_blank')}
                        >
