...
export function StudyResults() {
  ...
  const routeState = location.state as { study?: Study; imgUrls?: Record<number, string> } | null

  const [study, setStudy] = useState<Study | null>(routeState?.study ?? null)
  const [err, setErr] = useState<string | null>(null)
  const [imgUrls] = useState<Record<number, string>>(routeState?.imgUrls ?? {})

  useEffect(() => {
    if (!study) setErr('This local study is no longer available')
  }, [study])
  ...
  function setFeedback(imageId: number, feedback: 0 | 1) {
    setErr(null)
    setStudy((prev) =>
      prev
        ? {
